...
class Ion_auth
{
  ...
  public function register_seller($identity, $password, $email, $referer,$additional_data = [], $group_ids = [])
  {
    $this->ion_auth_model->trigger_events('pre_account_creation');

    $email_activation = $this->config->item('email_activation', 'ion_auth');

    $id = $this->ion_auth_model->register_reseller($identity, $password, $email, $referer,$additional_data,$group_ids);
