          ...
          <form class="theme-form" action="<?= base_url('users/refer/save') ?>" method="post">
            <div class="form-row">
              <?php if ($this->session->userdata('refferemail')) { ?>
                <input type="hidden" name="email_refer" value="<?= $this->session->userdata('refferemail') ?>">
                <div class="col-md-12">
                  <input type="email" disabled class="form-control" name="email_refer" id="email" value="<?= $this->session->userdata('refferemail') ?>" placeholder="Enter Email userRefer" required>
