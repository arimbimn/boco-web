...
public function identity_check_reseller($identity = '')
	{
		//$this->trigger_events('identity_check');
	    ...
	    return $this->db->select($this->identity_column)->where($this->identity_column, $identity)
            ->join('tb_reseller','users.id = tb_reseller.user_id')
			->limit(1)
			->count_all_results($this->tables['users']) > 0;
	}
	/**
...
public function register_reseller_old($identity, $password, $email ,$referer,$additional_data = [], $groups = [])
	{
		$this->trigger_events('pre_register');

		$manual_activation = $this->config->item('manual_activation', 'ion_auth');

		if ($this->identity_check_reseller($identity)) {
			$this->set_error('account_creation_duplicate_identity');
			return FALSE;
		} else if (!$this->config->item('reseller_group', 'ion_auth') && empty($groups)) {
			$this->set_error('account_creation_missing_default_group');
			return FALSE;
		}

		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'], ['name' => $this->config->item('reseller_group', 'ion_auth')], 1)->row();
		if (!isset($query->id) && empty($groups)) {
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
		}
		...
		$data_reseller = [
			...
			'refertouser_id' => ($referer != '' || $referer != 0 ? $referer : null)
		];
		$this->db->insert('tb_reseller', $data_reseller);
		// add in groups array if it doesn't exists and stop adding into default group if default group ids are set
		if (isset($default_group->id) && empty($groups)) {
			$groups[] = $default_group->id;
		}
	...
	public function register_reseller($identity, $password, $email ,$referer,$additional_data = [], $groups = [])
	{
		$this->trigger_events('pre_register');

		$manual_activation = $this->config->item('manual_activation', 'ion_auth');
        //$tes=$this->identity_check($identity);
        //var_dump($tes);exit();
		if ($this->identity_check_reseller($identity)) {
			$this->set_error('account_creation_duplicate_identity');
			return FALSE;
        ...
        $cekID=$this->cek_username($user_data['username'],$user_data['email'],$user_data['phone']);
        if($cekID){
            $id=$cekID->id;
            /*update user member exist*/
            $this->db->where('id', $cekID->id);
	...
	function cek_username($username,$mail,$hp){
	    $this->db->select('id');
        $this->db->where('username', $username);
        $this->db->or_where('email', $mail);
        $this->db->or_where('phone', $hp);
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$identity_col = $this->identity_column;
		$query_email = $this->db->get_where('users', ['email' => $identity])->row();
		$query_username = $this->db->get_where('users', ['username' => $identity])->row();
		$query_phone = $this->db->get_where('users', ['phone' => $identity])->row();
		if (!empty($query_email)) {
			$identity_col = 'email';
		} else if (!empty($query_username)) {
			$identity_col = 'username';
		} else if (!empty($query_phone)) {
			$identity_col = 'phone';
		}

		$query = $this->db->select('email, id, password, active, last_login, reseller,sts_reseller,username')
			->where($identity_col, $identity)
			//->where('reseller', null)
			->limit(1)
			->order_by('id', 'desc')
			->get($this->tables['users']);
		...
		if ($query->num_rows() === 1) {
			...
			if ($this->verify_password($password, $user->password, $identity)) {
				...
				/*ebe tambah session username upline utk menampilkan username/kodevoucher pada saat cekout*/
                $sts_reseller = (strlen($user->sts_reseller) == 0) ? 0 : $user->sts_reseller;
                if($user->reseller == '1' && $sts_reseller == 0){
				//var_dump($sts_reseller);exit();
                    //$cek_pembelian=$this->cek_pembelian_online($user->id);
                    //if(!$cek_pembelian){
                        $username_upline=$this->get_username_reseller_upline($user->id);
                        if($username_upline){
                            $upline_arr=[
                                'reselleruser' => $username_upline   
                            ];
							//var_dump($upline_arr);exit();
                            if($username_upline != 'PUSAT'){
                                $this->session->set_userdata($upline_arr);
                            }
                        }
                   // }
                }/*else if($user->reseller == '1' && $sts_reseller == 1){
                   $cek_pembelian=$this->cek_pembelian_online($user->id);
                    if(!$cek_pembelian){
                        $username_upline=$this->get_username_reseller_upline($user->id);
                        if($username_upline){
...
//var_dump($this->session->all_userdata());exit(); //ebe
				return TRUE;
			}
		}
	...
	public function login_reseller($identity, $password, $remember = FALSE)
	{
		$this->trigger_events('pre_login');
...
//var_dump($this->session->all_userdata());exit(); //ebe
	...
	function cek_pembelian_online($id){
	    $this->db->select('b.username');
        $this->db->join('users b', 'a.user_id  = b.id');
        $this->db->join('order c', 'a.user_id  = c.user_id');
        $this->db->where('a.user_id', $id);
        $this->db->where('b.reseller', '1');
        $this->db->where('b.sts_reseller', '1');
        $this->db->where('c.offline', 0);
        $this->db->where('c.status_bayar', 'PAID');
        $this->db->where('date(c.created_at) >=', '2023-05-03'); //tgl_launching reseller
        $this->db->or_where('a.user_id', $id);
        $this->db->where('b.reseller', '1');
        $this->db->where('b.sts_reseller', '1');
        $this->db->where('c.offline', 0);
        $this->db->where('c.status_bayar', 'SETTLED');
        $this->db->where('date(c.created_at) >=', '2023-05-03'); //tgl_launching reseller
	...
	function get_username_reseller_upline($id){
	    $this->db->select('b.username');
	    $this->db->join('users b', 'a.refertouser_id  = b.id');
        $this->db->where('a.user_id', $id);
        $query = $this->db->get('tb_reseller a');
        //return $query->row()->saldo;
		if ($query->num_rows() > 0) {
	...
	public function set_session($user)
	{
		...
		$session_data = [
			...
			'reseller'                 => $user->reseller, //ebe
			'username_nya'             => $user->username, //ebe
		];
	...
	function getIdUser($id)
  {
    ...
    if ($query->num_rows() > 0) {
  	...
  	function getIdUser_reseller_active($id)
  {
    $this->db->select('id');
    $this->db->where('username', $id);
	$this->db->where('reseller', '1');
	$this->db->where('sts_reseller', '1');
    $query = $this->db->get('users');
	//echo $this->db->last_query();
	//exit();
    if ($query->num_rows() > 0) {
  	...
  	function validate_reseller($val,$field)
  {
    $this->db->select('a.user_id');
    $this->db->join('users b', 'a.user_id  = b.id');
    if($field == 'phone'){
        $this->db->where('phone', $val);
    }else if($field == 'email'){
 ...
 function validate_reseller_edit_user($val,$field,$id)
  {
    $this->db->select('a.user_id');
    $this->db->join('users b', 'a.user_id  = b.id');
    if($field == 'phone'){
  ...
  function cek_status_singlepackage($id)
    {
        date_default_timezone_set('Asia/Jakarta');
        $tgl_sekarang=date("Y-m-d");
        $this->db->select('a.voucher_id');
        $this->db->join('users b', 'a.user_id  = b.id');
        $this->db->where('b.username', $id);
		$this->db->where('b.sts_reseller', '1');
		$this->db->where('a.voucher_id',118);
		$this->db->where('a.is_used',0);
		$this->db->where('date(DATE_ADD(a.created_at, INTERVAL 6 DAY)) <',$tgl_sekarang);
		$this->db->limit(1);
		/*$query=$this->db->get('voucher_used a');
		echo $this->db->last_query();
	    exit();*/
        return $this->db->get('voucher_used a')->row();
    }
}
